public class Stock4 {
    public int maxProfit1(int k, int[] prices) {
        ...
        int[][] minPri = new int[len][len];
        int[][] maxPro = new int[len][len];

        for (int i = 0; i < len; i++) {
            minPri[i][i] = prices[i];
            for (int j = i + 1; j < len; j++) {
                minPri[i][j] = Math.min(minPri[i][j - 1], prices[j]);
            }
        }
        ...
        for (int i = 0; i < len; i++) {
            maxPro[i][i] = 0;
            for (int j = i + 1; j < len; j++) {
                maxPro[i][j] = Math.max(maxPro[i][j - 1], prices[j] - minPri[i][j - 1]);   
            }
        }

        int[][] kPro = new int[len][k];

        for (int i = 1; i < len; i++) {
            kPro[i][0] = maxPro[0][i];
            for (int j = 0; j < i; j++) {
                for (int t = 1; t < k; t++) {
                    kPro[i][t] = Math.max(kPro[i][t], kPro[j][t - 1] + maxPro[j + 1][i]);
                }
            }
        }

        int result = kPro[len - 1][0];
        for (int i = 1; i < k; i++) {
            result = Math.max(result, kPro[len - 1][i]);
        }
        return result;
    }
    ...
    public int backtrack(int[][] profit, int depth, int start, int end, int sum) {
        if (depth == 1) {
            return sum + profit[start][end];
        }

        int maxPro = sum + profit[start][end];
        for (int i = start; i < end; i++) {
            sum += profit[start][i];
            maxPro = Math.max(maxPro, backtrack(profit, depth - 1, i + 1, end, sum));
            sum -= profit[start][i];
        }
        return maxPro;
    }
    ...
    public static void main(String[] args) {
        ...
        Stock4 so = new Stock4();
        System.out.println(so.maxProfit(3, p));
        System.out.println(so.maxProfit(100, a));
    }
}